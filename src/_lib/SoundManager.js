class SoundManager {
  ...
  playFootstep(options = {}) {
    ...
    const randomizedVolume =
      baseVolume * (1 - volumeVariation + Math.random() * volumeVariation * 2);
  ...
  /**
   * Set up event listeners for combat sound effects
   */
  setupCombatSoundEvents() {
    // Listen for combat hit events
    document.addEventListener('combat_hit', (e) => {
      const { attacker, target, attackType } = e.detail;

      // Only play sounds if player is attacking the enemy
      if (
        attacker &&
        attacker === window.PLAYER &&
        target &&
        target === window.ENEMY
      ) {
        // Play appropriate sound based on attack type
        if (attackType === 'kick') {
          this.playSound('kickSound', { volume: 0.5 });
        } else {
          // For all other attacks (slash, slash2, spinAttack, etc.)
          this.playRandomSwordSlash({ volume: 0.5 });
        }
      }
    });
  }
